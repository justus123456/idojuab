...
-- Email replies are sent only through the authenticated server endpoint and retained for customer-service history.
create table if not exists public.message_replies (
  id bigint generated by default as identity primary key,
  message_id bigint not null references public.messages(id) on delete cascade,
  recipient_email text not null,
  reply_body text not null check (char_length(reply_body) <= 2000),
  sent_by bigint references public.users(id),
  sent_at timestamptz not null default now()
);
create index if not exists message_replies_message_sent_idx on public.message_replies (message_id, sent_at desc);
alter table public.message_replies enable row level security;
drop policy if exists "message replies admin read" on public.message_replies;
create policy "message replies admin read" on public.message_replies for select to authenticated using (public.is_admin());
-- No browser insert, update, or delete policies: the server reply endpoint is the only writer.