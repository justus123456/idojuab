(function () {
  ...
  if (!db) return;
  const state = { prices: [], customers: [], orders: [], items: [], messages: [], settings: {}, faqs: [] };
  ...
  async function load() {
    const results = await Promise.all([
      ...
      db.from('messages').select('*').order('id', { ascending: false }), db.from('business_settings').select('*'), db.from('faqs').select('*').order('sort_order'), db.from('audit_logs').select('*').order('created_at', { ascending: false }).limit(100)
    ]);
    ...
    state.messages = results[4].data || []; state.settings = Object.fromEntries((results[5].data || []).map((r) => [r.key, r.value])); state.faqs = results[6].data || [];
    render(results[7].data || []); fillPriceSelect();
  }
  function render(audits) {
    ...
    renderCustomers(); renderOrders(); renderDashboard(); renderSettings(); renderFaqs(); renderAudits(audits);
  ...
  function renderOrders() {
    ...
    state.orders.filter((o) => {
      ...
      const row = body.insertRow(); rowValues(row, [o.ticket_number, o.customers?.full_name, o.customers?.phone, o.status, money(o.total), money(Number(o.total) - Number(o.amount_paid)), o.payment_status, o.expected_collection_at ? new Date(o.expected_collection_at).toLocaleString() : '']);
  ...
  function renderSettings() { if (el('setting-hours')) el('setting-hours').value = state.settings.business_hours || ''; if (el('setting-payment')) el('setting-payment').value = state.settings.payment_instructions || ''; if (el('setting-whatsapp')) el('setting-whatsapp').value = state.settings.whatsapp_number || ''; }
  ...
  async function notifyCustomer(order) { const balance = Math.max(0, Number(order.total) - Number(order.amount_paid)); const note = `Hello ${order.customers?.full_name || 'Customer'}, your laundry order ${order.ticket_number} is ready for collection. Your outstanding balance is ${money(balance)}. You can collect during our business hours.`; try { await navigator.clipboard.writeText(note); message('Notification copied. Paste it into WhatsApp.'); } catch { window.prompt('Copy notification:', note); } }
  ...
  function filterPriceRows() {
    ...
    el('order-form')?.addEventListener('submit', async (event) => { event.preventDefault(); if (!state.pending.length) return message('Add at least one item first.', true); const t = totals(); let result = await db.from('customers').upsert({ full_name: el('order-customer-name').value.trim(), phone: el('order-customer-phone').value.trim(), email: el('order-customer-email').value.trim().toLowerCase() || null }, { onConflict: 'phone' }).select().single(); if (result.error) return message(result.error.message, true); const ticket = `LD-${Date.now().toString().slice(-6)}`; result = await db.from('orders').insert({ ticket_number: ticket, customer_id: result.data.id, subtotal: t.subtotal, discount: t.discount, total: t.total, amount_paid: t.paid, payment_status: t.paid >= t.total ? 'paid' : t.paid ? 'partial' : 'unpaid', created_by: (await profile()).profile?.id || null, expected_collection_at: el('order-collection-date').value ? new Date(el('order-collection-date').value).toISOString() : null, notes: el('order-notes').value.trim() }).select().single(); if (result.error) return message(result.error.message, true); const items = state.pending.map((item) => ({ ...item, order_id: result.data.id })); result = await db.from('order_items').insert(items); if (result.error) return message(result.error.message, true); await audit('Created order', 'order', result.data.id, null, { ticket_number: ticket, total: t.total }); event.target.reset(); state.pending = []; renderPending(); await load(); message(`Order ${ticket} created.`); });
    el('settings-form')?.addEventListener('submit', async (event) => { event.preventDefault(); const rows = [['business_hours', el('setting-hours').value], ['payment_instructions', el('setting-payment').value], ['whatsapp_number', el('setting-whatsapp').value]]; for (const [key, value] of rows) { const result = await db.from('business_settings').upsert({ key, value }); if (result.error) return message(result.error.message, true); } await audit('Updated business settings', 'settings', 'business_settings', null, Object.fromEntries(rows)); await load(); message('Settings saved.'); });
